...
//Precondition: None
//Postcondition: Allocate space for a string object that represents the empty string. The string will have capacity 7 and size 0 by default. A copy of the address of the opaque object will be returned on success and NULL on failure.

MY_STRING my_string_init_default(void);

//Precondition: c_string is a valid null terminated c-string.
//Postcondition: Allocate space for a string object that represents a string with the same valuse as the given c-string. The capacity of the string object will be set to be one greater than is required to hold the string. As an example, the string "the" would set capacity at 4 instead of 3. A copy of the address of the opaque object will be returned on success and NULL on failure.
...
//Postcondition: Returns an integer less than zero if the string represented by hLeft_string is lexicographically smaller than hRight_string. If one string is a prefix of the other string, then the shorter strin is considered to be the smaller one (app is less than apple). Returns 0 if the strings are the same and returns a number greater than zero if the string represented by hLeft_string is bigger than hRight_string.

int my_string_compare(MY_STRING hLeft_string, MY_STRING hRight_string);

//Precondition: hMy_string is the handle to a valid My_string object.
//Postcondition: hMy_string will be the handle of a string object that contains the next string from the file stream fp according to the following rules.
// 1) Leading whitespace will be ignored.
// 2) All characters (after the first non-whitespace character is obtained and included) will be added to the string until a stopping condition is met. The capacity of the string will continue to grow as needed until all characters are stored.
